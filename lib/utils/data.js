...
export const decode = data => {
  try {
    ...
    const skills = Array.isArray(objStr.PathOfBuilding.Skills.Skill) ? objStr.PathOfBuilding.Skills.Skill : [objStr.PathOfBuilding.Skills.Skill]; // Pull the data we need
    ...
    return {
      ...
      skills: skills.filter(s => s['@_mainActiveSkill'] === 1 && !s['@_source']).map(s => {
        let gems = s.Gem.length ? s.Gem.map(g => ({
          name: g['@_nameSpec'],
          id: g['@_gemId'],
          skillId: g['@_skillId'],
          checked: false,
